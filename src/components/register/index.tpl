<section class="register-page">
    <a className="close" onClick={>navigate("/vote/questions")}>
    ...
    <div class="page-grid">
        <div class="link" data-label="links">
            <div class="qrcode"></div>
            <div class="url">telenor.team/vote</div>
        </div>
        <div class="voters-list">
            {::votersList().map(voter =>
                <div data-id={voter.id} className="voter" style={{animationDelay: (Math.random()*1992 | 0) + "ms"}}></div>
            )}
        </div>
        <div class="connected-count">
            Connected: {::votersList().length}
        </div>
    </div>
</section>